...
let _jobSeq = state.jobs.length
let _appSeq = state.applications.length
let _msgSeq = state.messages.length
let _intSeq = state.interviews.length
let _notifSeq = state.notifications.length

export function nextId(prefix) {
  switch (prefix) {
    case 'job_': _jobSeq++; return `job_${_jobSeq}`
    case 'app_': _appSeq++; return `app_${_appSeq}`
    case 'msg_': _msgSeq++; return `msg_${_msgSeq}`
    case 'int_': _intSeq++; return `int_${_intSeq}`
    case 'notif_': _notifSeq++; return `notif_${_notifSeq}`
